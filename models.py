# The DummyLM serves as a dummy language model to test the training pipeline
# The GPT model is from the minGPT-flax project by @brentyi:
# https://github.com/brentyi/minGPT-flax/blob/master/mingpt/model.py
...
class GPTConfig:
    ...
    # Enable attention chunking to trade runtime for memory efficiency. We 
    # implement an approach similar to the algorithm presented here:
...
# GPT quote: "Since layernorm is used extensively throughout the model, a 
# simple weight initialization of N(0,0.02) was sufficient."
...
class DummyLM(nn.Module):
    """Demo Large Language Model

    This is a demo language model that is used to test the training pipeline.
    """
    config: GPTConfig
    @nn.compact
    def __call__(self, x: jnp.ndarray):
        ...
        # Compute logtis
        logits = DenseWithInit(features=self.config.vocab_size,
                               use_bias=False
                              )(token_embeddings)
...
class MultiheadedCausalSelfAttention(nn.Module):
    """A simple causal self-attention module, with standard K/Q/V + output 
    projection matrices.
    """
    ...
    resid_pdrop: float
    attn_pdrop: float
    embd_dim: int  # Used for assertion on input shape.
    ...
    # type: ignore
        ...
        # Apply linear mappings to compute key, query, values for all heads in 
        # the batch.
        def kqv_linear_map(x: jnp.ndarray) -> jnp.ndarray:
            return DenseWithInit(features=C)(x).reshape(
                       (B, T, self.n_head, head_size)
                   )
        ...
        # Vectorize each attention head + the batch axis, passing each a unique 
        # dropout key. 
        # TODO: it should be straightforward to fold the vmapped axes into the
        # self-attention einsums; this would eliminate all of the PRNG key 
        # splitting.
        if deterministic:
            # In deterministic mode, the dropout PRNG key is not actually used, 
            # so we don't require that it's passed in.
            dropout_key = jax.random.PRNGKey(0)
        else:
            dropout_key = self.make_rng("dropout")
        dropout_key = jax.random.split(dropout_key, B)
        dropout_key = \
            jax.vmap(lambda k: jax.random.split(k, self.n_head))(dropout_key)
...
class GPTBlock(nn.Module):
    ...
    # type: ignore
        ...
        def mlp(x: jnp.ndarray) -> jnp.ndarray:
            ...
            x = nn.Dropout(rate=self.config.resid_pdrop,
                           deterministic=deterministic
                          )(x)
            return x
...
class GPT(nn.Module):
    ...
    # type: ignore
        ...
        pos_embedding_variable = self.variable(
            "params",
            "position_embeddings",
            # TODO: get intuition for why this is initialized to zero? 
            # why not random normal?
            jnp.zeros,  
            # shape arg for jnp.zeros
            (self.config.block_size, self.config.embd_dim),  
        )
        ...
        x = nn.Dropout(rate=self.config.embd_pdrop, 
                       deterministic=deterministic
                      )(x)
        ...
        # Andrej's implementation also (optionally) computes a loss here, but 
        # this seems like a weird place to do that.
        assert logits.shape == (B, T, self.config.vocab_size)
        return logits