# The attention module is from the minGPT-flax project by @brentyi: 
# https://github.com/brentyi/minGPT-flax/blob/master/mingpt/attention.py
...
def _dropout(x: jnp.ndarray, rate: float, key: jax.random.KeyArray
) -> jnp.ndarray:
    """Functional dropout implementation. In contrast to the flax.linen module, 
    this can be used inside of standard JAX function transforms.
...
def causal_self_attention_naive(
    ...
    *,
    ...
    """Simple causal self-attention implementation. This is fast, but requires 
    quadratic memory.

    k, q, and v should all be arrays of shape (token count, channel/head size). 
    Support for batch axes or multiple heads is simple with a vmap.
    """

    # Check shapes. Note that this is actually a generalized attention 
    # implementation, but our asserts assume self-attention and are overly 
    # restrictive!
    T, C = k.shape
...
def causal_self_attention_chunked(
    ...
    *,
    ...
    """Self-attention with chunking for both the queries and the keys. This may
    be slightly slower than the naive implementation, but requires 
    significantly less memory. Based on the approach described in [1], but 
    modified for masking + dropout.
    ...
    k, q, and v should all be arrays of shape (token count, channel/head size). 
    A batch axis can be added with a vmap."""
    ...
    # Division by square root of the head size results in better gradients for 
    # softmax (more values on the locally linear area).
    q = q / math.sqrt(C)
    ...
    *,
    ...
    def summarize_chunk(
        ...
        *,
        ...
        # Dropout.
        if not deterministic:
            exp_weights = _dropout(exp_weights, rate=pdrop,
                                   key=chunk_dropout_key
                                  )
    ...
    out = jnp.sum(reweighted_values, axis=0) / (
        # Due to dropout, the summed weights will sometimes (very rarely) total 
        # to 0. We add a small epsilonto prevent NaNs.
        #
        # In general this indicates that adding dropout to the the chunked
        # self-attention implementation may lead to operations that are not 
        # numerically stable; should be revisited for any real applications.
